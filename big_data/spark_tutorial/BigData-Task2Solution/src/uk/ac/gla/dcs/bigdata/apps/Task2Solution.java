...
import uk.ac.gla.dcs.bigdata.functions.map.StopwordFilterMap;
...
public class Task2Solution {
	...
	public static void main(String[] args) {
				...
				// Lets re-use some code from Tutorial 3-2 to get stopwords
				...
				// We want to collect information regarding how many games mention co-op gameplay or a story, so lets use accumulators for that
				LongAccumulator coopCountAccumulator = spark.sparkContext().longAccumulator();
				LongAccumulator storyAccumulator = spark.sparkContext().longAccumulator();
				
				// As a first step, lets remove stopwords from the detailed description for each game
				StopwordFilterMap stopwordFilterMap = new StopwordFilterMap(broadcastStopwords, coopCountAccumulator, storyAccumulator);

				
				// Apply our map function
				Dataset<SteamGameStats> steamGamesDescFiltered = steamGames.map(stopwordFilterMap, steamGameStatsEncoder);
				...
				List<SteamGameStats> games = steamGamesDescFiltered.collectAsList();
				
				// Now that an action has been called, we can collect the accumulator data
				long gamesMentionedCoop = coopCountAccumulator.value();
				long gamesMentionedStory = storyAccumulator.value();
				...
				for (SteamGameStats game : games) {
					System.out.println(game.getDetaileddescrip());
				}
				System.out.println("Games Mentioning Co-Op: "+gamesMentionedCoop);
				System.out.println("Games Mentioning Story: "+gamesMentionedStory);
